            ...
            if kiki.wkbType()==1 or kiki.wkbType()==2 or kiki.wkbType()==3 or kiki.wkbType()==5 or kiki.wkbType()==6:
                ...
                if self.dlg.tampon.isChecked():
                        ...
                        for feat in kiki.getFeatures():
                                ...
                                if val16 == "":
                                        kiki.changeAttributeValue(feat.id(),16,"")
                                else :
                                        kiki.changeAttributeValue(feat.id(),16,val16)
                                kiki.changeAttributeValue(feat.id(),17,val17)
                                if val18 == "":
                                        kiki.changeAttributeValue(feat.id(),18,"")
                                else :
                                        kiki.changeAttributeValue(feat.id(),18,val18)
                                kiki.changeAttributeValue(feat.id(),19,val19)
                                if val20 == "":
                                        kiki.changeAttributeValue(feat.id(),20,"")
                                else :
                                        kiki.changeAttributeValue(feat.id(),20,val20)
                                kiki.changeAttributeValue(feat.id(),21,val21)
                                if val22 == "":
                                        kiki.changeAttributeValue(feat.id(),22,"")
                                else :
                                        kiki.changeAttributeValue(feat.id(),22,val22)                                
                else :
                        for feat in layer.getFeatures():
                                layer.changeAttributeValue(feat.id(),0,feat.id())
                                if val1 == "":
                                ...
                                if self.get_communes()=="":
                                        ...
                                        layer.changeAttributeValue(feat.id(),15, val15)
                                if val16 == "":
                                        layer.changeAttributeValue(feat.id(),16,"")
                                else :
                                        layer.changeAttributeValue(feat.id(),16,val16)
                                layer.changeAttributeValue(feat.id(),17,val17)
                                if val18 == "":
                                        layer.changeAttributeValue(feat.id(),18,"")
                                else :
                                        layer.changeAttributeValue(feat.id(),18,val18)
                                layer.changeAttributeValue(feat.id(),19,val19)
                                if val20 == "":
...
# permet de zipper les fichiers de la couche de traitement ...   
    def archive (self, path):    
        ...
        if f_shpPath.exists() and f_shxPath.exists() and f_dbfPath.exists():
            ...
            return
            self.iface.messageBar().pushMessage(u'GéoMCE',u'Il manque un fichier' + f_shp + ' ou  '+f_dbf+ ' ou '+ f_shx +' pour créer une archive importable dans GéoMCE. Opération annulée', level=Qgis.Critical, duration=3) 
        if f_cpgPath.exists() :
...
# ... et des couches fusionnees
    def archive_2 (self, path):    
        chemin_couche = self.dlg.mQgsFileWidget.text()
        chemin_couche_2 = os.path.splitext(chemin_couche)[0]
        ...
        f_shp = chemin_couche_2 + '.shp'
        f_shpPath = Path(f_shp)
        f_shx = chemin_couche_2 + '.shx'
        f_shxPath = Path(f_shx)
        f_cpg = chemin_couche_2 + '.cpg'
        f_cpgPath= Path(f_cpg)
        f_dbf = chemin_couche_2 + '.dbf'
        f_dbfPath = Path(f_dbf)
        f_prj = chemin_couche_2 + '.prj'
        f_prjPath = Path(f_prj)
        if f_shpPath.exists() and f_shxPath.exists() and f_dbfPath.exists():
            archive.write(f_shp,basename(f_shp))
            archive.write(f_shx,basename(f_shx))
...
#... et des couches fusionnees        
 ...
 #efface completement le formulaire
    def eff_formulaire(self):
        ...
        self.dlg.surf_prescrite.clear()
        self.dlg.lin_prescrit.clear()
        self.dlg.nb_point_prescrit.clear()
        self.champs()

 #remplit les listes deroulantes 
    def champs(self):
        categorie = [u'E - Évitement',u'|-E1 - Évitement « amont » (stade anticipé)',u'|--E1-1 - Phase de conception du dossier de demande',u'|---E1-1-a - Évitement des populations connues d\'espèces protégées ou à fort enjeux et/ou de leurs habitats',u'|---E1-1-b - Évitement des sites à enjeux environnementaux et paysagers majeurs du territoire',u'|---E1-1-c - Redéfinition des caractéristiques du projet',u'|---E1-1-z - Autre : à préciser',u'|-E2 - Évitement géographique',u'|--E2-1 - Phase travaux',u'|---E2-1-a - Balisage préventif divers ou mise en défens ou dispositif de protection d\'une station d\'une espèce patrimoniale, d\'un habitat d\'une espèce patrimoniale, d\'habitats d\'espèces ou d\'arbres remarquables',u'|---E2-1-b - Limitation / positionnement adapté des emprises des travaux',u'|---E2-1-c - Respect des prescriptions d\'un APG (à préciser)',u'|---E2-1-z - Autre : à préciser',u'|--E2-2 - Phase exploitation / fonctionnement',u'|---E2-2-a - Balisage préventif divers ou mise en défens ou dispositif de protection d\'une station d\'une espèce patrimoniale, d\'un habitat d\'une espèce patrimoniale, d\'habitats d\'espèces ou d\'arbres remarquables',u'|---E2-2-b - Éloignement du projet vis-à-vis des populations humaines et/ou sites sensibles',u'|---E2-2-c - Mesure des documents de planification délimitant des zones et affectant les sols de manière à éloigner les populations humaines sensibles, application de marges de recul (urbanisations futures)',u'|---E2-2-d - Mesure d\'orientation d\'une installation ou d\'optimisation de la géométrie du projet',u'|---E2-2-e - Limitation (/ adaptation) des emprises du projet',u'|---E2-2-f - Positionnement du projet sur un secteur de moindre enjeu',u'|---E2-2-g - Respect des prescriptions d\'un APG (à préciser)',u'|---E2-2-z - Autre : à préciser',u'|-E3 - Évitement technique',u'|--E3-1 - Phase travaux',u'|---E3-1-a - Absence de rejet dans le milieu naturel (air, eau, sol, sous-sol)',u'|---E3-1-b - Respect des prescriptions d\'un APG (à préciser)',u'|---E3-1-z - Autre : à préciser',u'|--E3-2 - Phase exploitation / fonctionnement',u'|---E3-2-a - Absence totale d\'utilisation de produits phytosanitaires et de tout produit polluant ou susceptible d\'impacter négativement le milieu',u'|---E3-2-b - Redéfinition / Modifications / adaptations des choix d\'aménagement, des caractéristiques du projet (à préciser par le maître d\'ouvrage)',u'|---E3-2-c - Respect des prescriptions d\'un APG (à préciser)',u'|---E3-2-z - Autre : à préciser',u'|-E4 - Évitement temporel',u'|--E4-1 - Phase travaux',u'|---E4-1-a - Adaptation de la période des travaux sur l\'année',u'|---E4-1-b - Adaptation des horaires des travaux (en journalier)',u'|---E4-1-c - Respect des prescriptions d\'un APG (à préciser)',u'|---E4-1-z - Autre : à préciser',u'|--E4-2 - Phase exploitation / fonctionnement',u'|---E4-2-a - Adaptation des périodes d\'exploitation / d\'activité / d\'entretien sur l\'année',u'|---E4-2-b - Adaptation des horaires d\'exploitation / d\'activité / d\'entretien (fonctionnement diurne, nocturne, tenant compte des horaires de marées)',u'|---E4-2-c - Respect des prescriptions d\'un APG (à préciser)',u'|---E4-2-z - Autre : à préciser',u'R - Réduction',u'|-R1 - Réduction géographique',u'|--R1-1 - Phase travaux',u'|---R1-1-a - Limitation / adaptation des emprises des travaux et/ou des zones d\'accès et/ou des zones de circulation des engins de chantier',u'|---R1-1-b - Limitation / adaptation des installations de chantier',u'|---R1-1-c - Balisage préventif divers ou mise en défens (pour partie) ou dispositif de protection d\'une station d\'une espèce patrimoniale, d\'un habitat d\'une espèce patrimoniale, d\'habitats d\'espèces ou d\'arbres remarquables',u'|---R1-1-d - Respect des prescriptions d\'un APG (à préciser)',u'|---R1-1-z - Autre : à préciser',u'|--R1-2 - Phase exploitation / fonctionnement',u'|---R1-2-a - Limitation (/ adaptation) des emprises du projet',u'|---R1-2-b - Balisage définitif divers ou mise en défens définitive (pour partie) ou dispositif de protection définitif d\'une station d\'une espèce patrimoniale, d\'un habitat d\'une espèce patrimoniale, d\'habitats d\'espèces ou d\'arbres remarquables',u'|---R1-2-c - Respect des prescriptions d\'un APG (à préciser)',u'|---R1-2-z - Autre : à préciser',u'|-R2 - Réduction technique',u'|--R2-1 - Phase travaux',u'|---R2-1-a - Adaptation des modalités de circulation des engins de chantier',u'|---R2-1-b - Mode particulier d\'importation de matériaux et/ou d\'évacuation des matériaux, déblais et résidus de chantier : transport fluvial, transport ferroviaire, etc',u'|---R2-1-c - Optimisation de la gestion des matériaux (déblais et remblais)',u'|---R2-1-d - Dispositif préventif de lutte contre une pollution et dispositif d\'assainissement provisoire de gestion des eaux pluviales et de chantier',u'|---R2-1-e - Dispositif préventif de lutte contre l\'érosion des sols',u'|---R2-1-f - Dispositif de lutte contre les espèces exotiques envahissantes (actions préventives et curatives)',u'|---R2-1-g - Dispositif limitant les impacts liés au passage des engins de chantier',u'|---R2-1-h - Clôture et dispositif de franchissement provisoires adaptés aux espèces animales cibles',u'|---R2-1-i - Dispositif permettant d\'éloigner les espèces à enjeux et/ou limitant leur installation',u'|---R2-1-j - Dispositif de limitation des nuisances envers les populations humaines',u'|---R2-1-k - Dispositif de limitation des nuisances envers la faune',u'|---R2-1-l - Maintien d\'un débit minimum « biologique » de cours d\'eau',u'|---R2-1-m - Maintien d\'une connexion latérale (espèces aquatiques)',u'|---R2-1-n - Récupération et transfert d\'une partie du milieu naturel',u'|---R2-1-o - Prélèvement ou sauvetage avant destruction de spécimens d\'espèces – Espèce(s) à préciser',u'|---R2-1-p - Gestion écologique temporaire des habitats dans la zone d\'emprise des travaux',u'|---R2-1-q - Dispositif d\'aide à la recolonisation du milieu',u'|---R2-1-r - Dispositif de repli du chantier',u'|---R2-1-s - Respect des prescriptions d\'un APG (à préciser)',u'|---R2-1-z - Autre : à préciser',u'|--R2-2 - Phase exploitation / fonctionnement',u'|---R2-2-a - Action sur les conditions de circulation (ferroviaire, routier, aérien, maritime)',u'|---R2-2-b - Dispositif de limitation des nuisances envers les populations humaines',u'|---R2-2-c - Dispositif de limitation des nuisances envers la faune',u'|---R2-2-d - Dispositif anti-collision et d\'effarouchement (hors clôture spécifique)',u'|---R2-2-e - Passage supérieur à faune / Ecopont (spécifique ou mixte)',u'|---R2-2-f - Passage inférieur à faune / Ecoduc (spécifique ou mixte)',u'|---R2-2-g - Dispositif complémentaire au droit d\'un passage faune (supérieur ou inférieur) afin de favoriser sa fonctionnalité',u'|---R2-2-h - Dispositif de franchissement piscicole',u'|---R2-2-i - Maintien d\'un débit minimum « biologique » de cours d\'eau',u'|---R2-2-j - Clôture spécifique (y compris échappatoire) et dispositif anti-pénétration dans les emprises',u'|---R2-2-k - Plantation diverses : sur talus type up-over (« tremplin vert ») ou visant la mise en valeur des paysages',u'|---R2-2-l - Installation d\'abris ou de gîtes artificiels pour la faune au droit du projet ou à proximité',u'|---R2-2-m - Dispositif technique limitant les impacts sur la continuité hydraulique',u'|---R2-2-n - Optimisation de la gestion des matériaux (déblais et remblais)',u'|---R2-2-o - Gestion écologique des habitats dans la zone d\'emprise du projet',u'|---R2-2-p - Respect des prescriptions d\'un APG (à préciser)',u'|---R2-2-q - Dispositif de gestion et traitement des eaux pluviales et des émissions polluantes',u'|---R2-2-z - Autre : à préciser',u'|-R3 - Réduction temporelle',u'|--R3-1 - Phase travaux',u'|---R3-1-a - Adaptation de la période des travaux sur l\'année',u'|---R3-1-b - Adaptation des horaires des travaux (en journalier)',u'|---R3-1-c - Respect des prescriptions d\'un APG (à préciser)',u'|---R3-1-z - Autre : à préciser',u'|--R3-2 - Phase exploitation / fonctionnement',u'|---R3-2-a - Adaptation des périodes d\'exploitation / d\'activité / d\'entretien sur l\'année',u'|---R3-2-b - Adaptation des horaires d\'exploitation / d\'activité / d\'entretien (fonctionnement diurne, nocturne, tenant compte des horaires de marées)',u'|---R3-2-c - Respect des prescriptions d\'un APG (à préciser)',u'|---R3-2-z - Autre : à préciser',u'C - Compensation',u'|-C1 - Création / Renaturation de milieux',u'|--C1-1 - Action concernant tous types de milieux',u'|---C1-1-a - Création ou renaturation d\'habitats et d\'habitats favorables aux espèces cibles et à leur guilde (à préciser)',u'|---C1-1-b - Aménagement ponctuel (abris ou gîtes artificiels pour la faune) complémentaire à une mesure C1.a ou à une mesure C2 ',u'|---C1-1-c - Respect des prescriptions d\'un APG (à préciser)',u'|---C1-1-z - Autre : à préciser',u'|-C2 - Restauration / Réhabilitation',u'|--C2-1 - Action concernant tous types de milieux',u'|---C2-1-a - Enlèvement de dispositifs d\'aménagements antérieurs (déconstruction) hors ouvrages en eau',u'|---C2-1-b - Enlèvement / traitement d\'espèces exotiques envahissantes (EEE)',u'|---C2-1-c - Etrépage / Décapage / Décaissement du sol ou suppression de remblais',u'|---C2-1-d - Réensemencement de milieux dégradés, replantation, restauration de haies existantes mais dégradées',u'|---C2-1-e - Réouverture du milieu par débroussaillage d\'espèces ligneuses, abattage d\'arbres, etc',u'|---C2-1-f - Restauration de corridor écologique',u'|---C2-1-g - Aménagement ponctuel (abris ou gîtes artificiels pour la faune) complémentaire à une autre mesure C2',u'|---C2-1-h - Respect des prescriptions d\'un APG (à préciser)',u'|---C2-1-z - Autre : à préciser',u'|--C2-2 - Actions spécifiques aux cours d\'eau (lit mineur + lit majeur), annexes hydrauliques, étendues d\'eau stagnantes et zones humides',u'|---C2-2-a - Reprofilage / Restauration de berges (y compris suppression des protections)',u'|---C2-2-b - Amélioration / entretien d\'annexes hydrauliques / décolmatage de fond et action sur la source du colmatage',u'|---C2-2-c - Reconnexion d\'annexes hydrauliques avec le cours d\'eau / reconnexion lit mineur/lit majeur / Restauration de zones de frayères',u'|---C2-2-d - Restauration des conditions hydromorphologiques du lit mineur de cours d\'eau',u'|---C2-2-e - Restauration des modalités d\'alimentation et de circulation de l\'eau au sein d\'une zone humide',u'|---C2-2-f - Restauration de ripisylves existantes mais dégradées',u'|---C2-2-g - Modification ou équipement d\'ouvrage existant',u'|---C2-2-h - Arasement ou dérasement d\'un obstacle transversal, d\'un seuil, d\'un busage',u'|---C2-2-i - Aménagement d\'un point d\'abreuvement et mise en défens des berges',u'|---C2-2-z - Autre : à préciser',u'|--C3-1 - Abandon ou changement total des modalités de gestion antérieures',u'|---C3-1-a - Abandon ou forte réduction de tout traitement phytosanitaire',u'|---C3-1-b - Abandon ou forte réduction de toute gestion : îlot de senescence, autre (à préciser)',u'|---C3-1-c - Changement des pratiques culturales par conversion de terres cultivées ou exploitées de manière intensive',u'|---C3-1-d - Respect des prescriptions d\'un APG (à préciser)',u'|---C3-1-z - Autre : à préciser',u'|--C3-2 - Simple évolution des modalités de gestion antérieures',u'|---C3-2-a - Modification des modalités de fauche et/ou de pâturage ou modification de la gestion des niveaux d\'eau',u'|---C3-2-b - Mise en place de pratiques de gestion alternatives plus respectueuses des milieux (à préciser par le maître d\'ouvrage)',u'|---C3-2-c - Modification des modalités de gestion de la fréquentation humaine',u'|---C3-2-d - Respect des prescriptions d\'un APG (à préciser)',u'|---C3-2-z - Autre : à préciser',u'A - Accompagnement',u'|-A1 - Préservation  foncière',u'|--A1-1 - Cas dérogatoire des LD ERC',u'|---A1-1-a - Acquisition de parcelle sans mise en œuvre d\'action écologique complémentaire',u'|--A1-2 - Site en bon état de conservation',u'|---A1-2-a - Acquisition de parcelle sans mise en œuvre d\'action écologique complémentaire. Le milieu acquis peut ne pas respecter la condition d\'équivalence écologique',u'|-A2 - Pérennité des mesures compensatoires C1 à C3 et A1',u'|--A2-1 - Pérennité des mesures compensatoires C1 à C3 et A1',u'|---A2-1-a - Mise en place d\'un outil réglementaire du Code de l\'Environnement ou du Code Rural et de la pêche maritime ou du Code de l\'Urbanisme : à préciser',u'|---A2-1-b - Rattachement du foncier à un réseau de sites locaux : à préciser',u'|---A2-1-c - Cession / rétrocession du foncier : à préciser',u'|---A2-1-d - Mise en place d\'obligations réelles environnementales',u'|-A3 - Réaménagement / rétablissement',u'|--A3-1 - Réaménagement / rétablissement',u'|---A3-1-a - Aménagement ponctuel (abris ou gîtes artificiels pour la faune)',u'|---A3-1-b - Aide à la recolonisation végétale',u'|---A3-1-z - Autre : à préciser',u'|-A4 - Financement',u'|--A4-1 - Financement intégral du maître d\'ouvrage',u'|---A4-1-a - Aide financière au fonctionnement de structures locales',u'|---A4-1-b - Approfondissement des connaissances relatives à une espèce ou un habitat impacté, aux paysages, à la qualité de l\'air et aux niveaux de bruit : à préciser',u'|---A4-1-c - Financement de programmes de recherche',u'|---A4-1-z - Autre : à préciser',u'|--A4-2 - Contribution à une politique publique',u'|---A4-2-a - Contribution financière au déploiement d\'actions prévues par un document couvrant le territoire endommagé',u'|---A4-2-b - Contribution au financement de la réalisation de document d\'action en faveur d\'une espèce ou d\'un habitat endommagé par le projet',u'|---A4-2-c - Financement de programmes de recherche',u'|---A4-2-z - Autre : à préciser',u'|-A5 - Actions expérimentales',u'|--A5-1 - Actions expérimentales',u'|---A5-1-a - Action expérimentale de génie-écologique',u'|---A5-1-b - Action expérimentale de renforcement de population ou de transplantation d\'individus / translocation manuelle ou mécanique',u'|---A5-1-z - Autre : à préciser',u'|-A6 - Action de gouvernance/ sensibilisation / communication ',u'|--A6-1 - Gouvernance',u'|---A6-1-a - Organisation administrative du chantier',u'|---A6-1-b - Mise en place d\'un comité de suivi des mesures',u'|---A6-1-z - Autre : à préciser',u'|--A6-2 - Communication, sensibilisation ou de diffusion des connaissances',u'|---A6-2-a - Action de gestion de la connaissance collective',u'|---A6-2-b - Déploiement d\'actions de communication',u'|---A6-2-c - Déploiement d\'actions de sensibilisation',u'|---A6-2-d - Dispositif de canalisation du public ou de limitation des accès',u'|---A6-2-z - Autre : à préciser',u'|-A7 - Mesure « paysage »',u'|--A7-1 - Mesure « paysage »',u'|---A7-1-a - Aménagements paysagers d\'accompagnement du projet dans les emprises et hors emprises',u'|-A8 - « Moyens » concourant à la mise en œuvre d\'une MC',u'|--A8-1 - « Moyens » concourant à la mise en œuvre d\'une MC',u'|---A8-1-z - A préciser',u'|-A9 - Autre',u'|--A9-1 - Autre',u'|---A9-1-a - Mesure d\'accompagnement ne rentrant dans aucune des catégories ci-avant A1 à A8 : à préciser',u'Z - Classe de mesure à préciser',u'|-Zz - Type de mesure à préciser',u'|--Zzz - Catégorie de mesure à préciser',u'|---Zzzz - Sous-catégorie de mesure à préciser']
        self.dlg.comboBox.addItems(categorie)
        cible = [u'Continuités écologiques',u'Bruit',u'Eau',u'Biens matériels',u'Equilibres biologiques',u'Espaces naturels, agricoles, forestiers, maritimes ou de loisirs',u'Facteurs climatiques',u'Faune et flore',u'Habitats naturels',u'Patrimoine culturel et archéologique',u'Population',u'Sites et paysages',u'Sol']
        ...
        surf_un = [u'Hectares', u'Ares',u'km²', u'm²']
        self.dlg.surf_unite.addItems(surf_un)        
        lin_un = [u'm', u'km']
        self.dlg.line_unite.addItems(lin_un)
        date_ech = [u'0',u'1',u'2',u'3',u'4',u'5',u'6',u'7',u'8',u'9',u'10',u'15',u'20',u'25',u'30',u'35',u'40',u'45',u'50']
        self.dlg.date_echeance.addItems(date_ech)

#fonctions relatives au code insee des communes concernees par des mesures
 ...
 #permet d actualiser la liste des couches pouvant etre fusionnes
    def listWidget(self):
...
#permet de charger les flus wfs du ceremadata qui alimentent geoportail
    def wfs(self):
 ...
 #permet de switecher les champs qui seront actifs si la saisie concerne un MAJ de donnees existantes dans geomce
    def maj_label(self):
        if self.dlg.maj.isChecked():
            self.dlg.label_4.hide()
            self.dlg.label_20.show()
        else :
            self.dlg.label_4.show()
            self.dlg.label_20.hide()
  ...
  #ouvre la boite de dialogue Aide/A propos
    ...
    def run(self):
        ...
        if check == 0:
            self.iface.messageBar().pushMessage(u'GéoMCE',u'Il faut charger des couches vecteurs pour ouvrir le plug-in', level=Qgis.Critical, duration=3)
            return
        else:
            self.dlg.show()
            self.compatibilite()
